 ...
 */
async function writeVersionSources(version: string, hashes: Record<NixPlatform, string>): Promise<void> {
...
// On first run (no existing versions tracked), only seed with the latest.
...
const missingVersions = earliest
  ? allNpmVersions.filter((v) => !existingVersions.has(v) && semver.order(v, earliest) >= 0)
  : [latestVersion];
...
// Format with oxfmt if available and configured. Skipped silently otherwise so
// the script stays usable without the formatter installed.
const oxfmtConfig = join(import.meta.dir, ".oxfmtrc.jsonc");
const oxfmtExists = await Bun.file(oxfmtConfig).exists();
if (oxfmtExists) {
  try {
    await $`oxfmt --config ${oxfmtConfig} versions/*.json`.quiet();
    console.log("Formatted with oxfmt");
  } catch {
    console.warn("oxfmt failed; leaving JSON unformatted");
  }
}
