{
  ...
  inherit (sources) version nativeHashes npmTarballHash nodeOptionalDepHashes;
  ...
  nativeBinary =
    if runtime == "native" && platform != null
    then
      fetchurl {
        url = nativeBinaryUrl;
  ...
  npmTarball =
    ...
    then
      fetchurl {
        ...
        sha256 = npmTarballHash;
      }
    else null;

  nodeOptionalDep =
    if runtime == "node" && nodePlatform != null
    then
      fetchurl {
...
in
  ...
  || throw "Native runtime not supported on ${stdenv.hostPlatform.system}. Supported: aarch64-darwin, x86_64-darwin, x86_64-linux, aarch64-linux";
    stdenv.mkDerivation {
      ...
      passthru = {
        updateScript = writeShellScript "codex-update" ''
          export PATH="${lib.makeBinPath [bash gh git jq nix nix-prefetch-scripts]}:$PATH"
          for candidate in "$PWD/pkgs/codex" "$PWD"; do
            if [ -f "$candidate/update.sh" ] && [ -f "$candidate/sources.json" ]; then
              exec bash "$candidate/update.sh"
            fi
          done
          echo "Could not locate pkgs/codex source directory (cwd: $PWD)" >&2
          exit 1
        '';
      };
      ...
      meta = with lib; {
        ...
        platforms =
          if runtime == "native"
          then ["aarch64-darwin" "x86_64-darwin" "x86_64-linux" "aarch64-linux"]
          else platforms.all;
