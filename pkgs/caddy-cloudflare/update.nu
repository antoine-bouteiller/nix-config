...
def fetch_latest_version []: nothing -> string {
  http get $"https://api.github.com/repos/($github_repo)/tags?per_page=1"
  | get 0.name
  | str replace -r '^v' ''
}
...
def main [] {
  ...
  # caddy.withPlugins is a FOD whose contents depend on the nixpkgs caddy
  # version, so the hash can drift even when the plugin version is
  # unchanged. Always recompute it.
  ...
  let build = (do {
    nix build $".#packages.($system).caddy-cloudflare" --no-link --impure
  } | complete)
  ...
  if $current_version == $latest_version and $current_hash == $new_hash {
    print $"caddy-cloudflare ($latest_version) already up to date \(hash unchanged\)"
  } else {
    print $"Updated caddy-cloudflare to version ($latest_version) \(hash: ($new_hash)\)"
  }
}
