...
These defaults target this machine's Azure OpenAI setup (base `config.toml` has `model_provider = "azure"`, key via `AZURE_OPENAI_API_KEY`). The model names are Azure **deployment** names, not OpenAI model IDs — `gpt-5.6-terra` for the reviewer and `gpt-5.5` as the cheaper summarizer workhorse. `model_provider = "azure"` is set explicitly so the profile can't silently fall back to a nonexistent OpenAI deployment. Adjust to whatever deployments the endpoint exposes. Note: `codex login status` reports "Not logged in" under Azure key auth — that only tracks ChatGPT login and is expected.
...
If either AI flags a security concern in the blind pass, escalate that issue immediately to external research. The peer review skill is agnostic about _which_ research tool to use — pick the best one available (web search, an MCP research tool, vendor docs, etc.). See @escalation-criteria.md.
